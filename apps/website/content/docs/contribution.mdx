...
# Windows
pnpm --filter=@read-frog/extension run dev
pnpm --filter=@read-frog/website run dev
```
...
If the remote main branch gets updated and creates conflicts in our PR, we can fix it by merging the remote branch first.
